...
# Работает только с базой stats_db, не изменяя другие базы данных.
...
# Проверка новых баз и обновление db_creation
START_TIME=$(date +%s)
...
if [ -n "$NEW_DBS" ]; then
  while IFS='|' read -r datname oid; do
    ...
    psql -h "$PGHOST" -d "$STAT_DB" -U "$PGUSER" -c "
      INSERT INTO db_creation (datname, creation_time)
      VALUES ('$datname', '$CREATION_TIME');
    " >> "$LOG_FILE" 2>&1
  done <<< "$NEW_DBS"
fi
END_TIME=$(date +%s)
...
# Обновляем db_size только для существующих баз, у которых нет актуального размера за последние 30 минут
START_TIME=$(date +%s)
# Получаем список баз, требующих обновления размера
UPDATE_DBS=$(psql -h "$PGHOST" -d "$STAT_DB" -U "$PGUSER" -t -A -c "
  WITH latest_sizes AS (
    SELECT s.datname, MAX(s.last_size_update) AS last_update
    FROM pg_statistics s
  ...
  SELECT datname
  FROM latest_sizes
  WHERE last_update IS NULL OR last_update < now() - interval '30 minutes';
" 2>>"$LOG_FILE")
...
if [ -n "$UPDATE_DBS" ]; then
  ...
  for datname in $UPDATE_DBS; do
    ...
    # Обновляем размер только для существующей базы
    psql -h "$PGHOST" -d "$STAT_DB" -U "$PGUSER" -c "
      UPDATE pg_statistics
      SET db_size = pg_database_size('$datname'),
          last_size_update = now()
      WHERE datname = '$datname'
        AND ts = (SELECT MAX(ts) FROM pg_statistics WHERE datname = '$datname');
    " >> "$LOG_FILE" 2>&1
    END_DB_TIME=$(date +%s)
